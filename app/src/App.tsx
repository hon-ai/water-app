...
export default function App() {
  ...
  const refreshStatus = useCallback(async () => {
    try {
      ...
      // Sidecar status — `null` when no sidecar object exists at
      // all (the spawn never succeeded; usually means `uv` isn't
      // on PATH). Treat that as "missing" for the uv-install
      // banner. `error`/`loading`/`ready` come from the running
      // process's health checks.
      setSidecarStatus(s.sidecar?.status ?? null);
    } catch {
      /* swallow */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setSceneAnchors((prev) => {
        // Avoid re-rendering when anchors haven't meaningfully
        // moved (skip the React commit when each (x, y) is within
        // 0.5 px of the previous frame). The ribbon's path
        // animation runs off `performance.now()`; we don't need
        // setState churn between layout shifts.
        if (prev.length !== anchors.length) return anchors;
        for (let i = 0; i < anchors.length; i++) {
          if (
            Math.abs(prev[i]!.x - anchors[i]!.x) > 0.5 ||
            Math.abs(prev[i]!.y - anchors[i]!.y) > 0.5
          ) {
            return anchors;
          }
        }
