...
- Orbit (`~/Desktop/Orbit`) — `frontend/src/services/openai.ts` for character-embodied + craft prompt patterns; `backend/app/agents/grammar_agent.py` for rule-based editor heuristics.
...
- Built as a ProseMirror decoration plugin (port the pattern from Orbit's `setHighlightedParagraph`, scoped to a character range instead of a node range).
...
| `passive_voice` | `(was|were|been|is|are|be) + past-participle` (ports Orbit's heuristic) | suggestion |
...
| `common_mistake` | `their is`, `could of`, `your welcome` (Orbit's table) | warning |
...
The `text_snippet` + `content_hash` columns mirror Orbit's two-layer anchor validation — re-runs locate the right span even if the writer's edits shift offsets.
...
### F.1 Lessons from Orbit

Orbit's prompts that landed (~30% by user judgment):
...
Each persona prompt gets a single-line "what this speaker *won't* do" block, ported from Orbit's blacklist pattern. Examples:
